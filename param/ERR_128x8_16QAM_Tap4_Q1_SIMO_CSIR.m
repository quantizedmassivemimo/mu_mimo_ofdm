function ERR_128x8_16QAM_Tap4_Q1_SIMO_CSIR(RunID)
  ...
  TxRx.Sim.SNR_dB_list = [0:2:12];    
  
  % -- system configuration
  TxRx.Modulation_order = 4; % Modulation scheme: BPSK (1), QPSK (2), 16-QAM (4), 64-QAM (6)
  TxRx.Nrx = 128;             % Number of receivers (base station antennas)
  ...
  % -- detector type
  TxRx.DetectorName = 'SIMO'; % 'Infty', 'MMSE', 'CSI'    
  TxRx.DetectorSoft = 'Yes'; % 'Yes', 'No
  TxRx.ChannelRescaling = 'None'; % 'None', 'Variance', 'Oracle'
  TxRx.CSIR = 'yes'; % 'yes', 'no'
  
  % -- quantizer
  TxRx.Q.Bins = 2^1; % number of bins
  TxRx.Q.Type = 'bins'; 
